...
class SubscriptionManager
{
     ...
     */
    public function updateFromCustomer(CustomerInterface $customer)
    {
        try {
            ...
            $this->api->updateSubscriberInList(
                $this->subscriberDataMapper->mapFromCustomer(
                    $customer,
                    $this->subscriberExists($customer->getEmail())
                )
            );
        } catch (Exception $e) {
            $this->logger->error('Failed to update Ecomail subscription.', [$e]);
        }
    }
